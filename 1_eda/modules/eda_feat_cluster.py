...
def cluster_features(corr_matrix, n_clusters=20, method="ward"):
    """
    ...
    n_clusters : int
        최종 클러스터 수. 기본 20
    method : str
        scipy linkage method. 기본 "ward"
        ("ward"는 분산 최소화, "complete"/"average"/"single" 등 사용 가능)
...
def cluster_target_correlation(xs_dict, ys_train, feat_cols, cluster_labels,
    ...
    xs_unit = xs_train.groupby(KEY_COL)[valid_feats].mean()
