"""
01_zit/bag_zit_eql/hpo.py — BagZITEQL(EQL φ) 병렬 HPO 튜너 (thin).
...
from modules.zit import BagZITEQLRegressor  # noqa: E402
...
def main() -> None:
    ap = argparse.ArgumentParser(description="BagZIT (EQL phi) parallel HPO worker.")
    ...
    objective = zit_objective.make_zit_objective(
        args, data,
        model_class=BagZITEQLRegressor,
        search=SEARCH,
    ...
    study = ph.make_study(args, db_path, study_meta={
        "model": "BagZITEQLRegressor (bag, EQL phi)",
        "search_space": SEARCH,
