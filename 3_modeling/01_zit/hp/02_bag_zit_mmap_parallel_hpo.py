...
Run command (PowerShell) -- 14 workers on one PC, n_jobs=1 (14 threads = 14 cores):
  ...
  python 3_modeling/01_zit/hp/02_bag_zit_mmap_parallel_hpo.py --enqueue-anchor --n-trials 0

  # Open 14 workers (or use a launcher). Only --worker-id changes; same --end-at to all.
  # All workers mmap-share ONE copy of pp.npy.
  python 3_modeling/01_zit/hp/02_bag_zit_mmap_parallel_hpo.py --worker-id w1 --n-trials 100000 --n-jobs 1 --n-startup-trials 12 --end-at 2026-06-05T09:00 > 4_output/logs/bag004_w1.log 2>&1
