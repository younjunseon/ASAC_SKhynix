"""
Parallel HPO worker for BagZIT (base / simplified phi update).

This is the simplified sibling of 02_bag_zit_eql_parallel_hpo.py. The EQL worker
overrode the phi M-step to use the Tweedie unit deviance (paper-matched); this
worker reverts that one part back to the module's base target:

  EQL worker (002):   phi_target = Tweedie unit deviance D_zeta(y, mu)        (paper)
  this worker (003):  phi_target = (y - mu)^2 / mu^zeta                       (base ZITboost)

So this worker uses modules.zit.BagZITboostRegressor *as-is* -- the same simplified
phi target that 01_zit_only_parallel_hpo.py uses -- with no local subclass.
3_modeling/modules/zit.py is not modified. Everything else (preprocessing incl. the
in-process median-imputation patch, search space, anchor, fold logic, tau_pi gate) is
identical to the EQL worker, so bag-zit-final-003 is a clean A/B against
bag-zit-eql-final-002: the only moving part is the phi update.

The BagZIT idea (same as 02_bag_zit.ipynb / the EQL worker):
...
Run command (PowerShell) -- 6 workers on one machine, n_jobs=3 each (6 * 3 = 18 threads):

  # One-time init. Enqueues bag-zit-final-001 best as anchor, then exits.
  python 3_modeling/01_zit/hp/02_bag_zit_parallel_hpo.py --enqueue-anchor --n-trials 0

  # Open 6 terminals. Only --worker-id changes; pass the same --end-at to all.
  python 3_modeling/01_zit/hp/02_bag_zit_parallel_hpo.py --worker-id w1 --n-trials 2000 --n-jobs 3 --n-startup-trials 12 --end-at 2026-06-02T09:00 > bag_w1.log 2>&1
  python 3_modeling/01_zit/hp/02_bag_zit_parallel_hpo.py --worker-id w2 --n-trials 2000 --n-jobs 3 --n-startup-trials 12 --end-at 2026-06-02T09:00 > bag_w2.log 2>&1
  python 3_modeling/01_zit/hp/02_bag_zit_parallel_hpo.py --worker-id w3 --n-trials 2000 --n-jobs 3 --n-startup-trials 12 --end-at 2026-06-02T09:00 > bag_w3.log 2>&1
  python 3_modeling/01_zit/hp/02_bag_zit_parallel_hpo.py --worker-id w4 --n-trials 2000 --n-jobs 3 --n-startup-trials 12 --end-at 2026-06-02T09:00 > bag_w4.log 2>&1
  python 3_modeling/01_zit/hp/02_bag_zit_parallel_hpo.py --worker-id w5 --n-trials 2000 --n-jobs 3 --n-startup-trials 12 --end-at 2026-06-02T09:00 > bag_w5.log 2>&1
  python 3_modeling/01_zit/hp/02_bag_zit_parallel_hpo.py --worker-id w6 --n-trials 2000 --n-jobs 3 --n-startup-trials 12 --end-at 2026-06-02T09:00 > bag_w6.log 2>&1
...
DEFAULT_EXP_ID = "bag-zit-final-003"
DEFAULT_USER = "jh"
...
#   (나쁜 영역만 잘라내고 anchor·상위 영역은 유지). tau_pi는 분류 게이트라 영향이 커 넓게 유지.
#   (001 분석이라 simplified phi인 이 003에 그대로 정합 — 002 EQL과 동일 search space로 A/B.)
BAG_ZIT_SEARCH = {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="BagZIT (base phi) parallel HPO worker.")
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Absolute wall-clock end datetime in ISO format, e.g. '2026-06-02T09:00'. "
...
def create_study(args: argparse.Namespace, db_path: Path) -> optuna.Study:
    ...
    study_meta = {
        "exp_id": args.exp_id,
        "user": args.user,
        "model": "BagZIT (base, simplified phi) parallel HPO worker",
