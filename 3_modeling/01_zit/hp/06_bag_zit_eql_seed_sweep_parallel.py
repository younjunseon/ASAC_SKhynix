...
Run command (PowerShell/cmd) from the project root:
  ...
  python 3_modeling/01_zit/hp/06_bag_zit_eql_seed_sweep_parallel.py --src-num auto --n-trials 0

  # 2) launch N independent workers (size N to RAM; ~5 GB peak/worker). Same shared study.
  python 3_modeling/01_zit/hp/06_bag_zit_eql_seed_sweep_parallel.py --src-num auto --worker-id w1 --n-jobs 2 > 4_output/logs/sweep_w1.log 2>&1
  ...  (w2 .. wN, identical except --worker-id)

  # 3) after seeds finish (or anytime): rebuild summary CSV + copy best seed -> best/
  python 3_modeling/01_zit/hp/06_bag_zit_eql_seed_sweep_parallel.py --src-num auto --summarize
"""
...
def _src_db_path(num, variant):
    ...
    return OUTPUT_DIR / "01_zit" / "bag_zit" / "hp" / num / f"optuna_jh_{name}.db", name
...
def load_source_and_data(args):
    ...
    if apply_median:
        ...
        wpath = CFG_PROJECT_ROOT / "3_modeling" / "01_zit" / "hp" / worker_fname
