"""
squeeze_extreme_v3.py
=====================

squeeze_extreme_v2.py의 die-level 버전.

v2와의 핵심 차이
----------------
- 메타러너를 **die 행**(unit당 4행)에서 학습한다. base 모델의 `*_die.csv`(die-level
  예측, `pred` 없으면 `prob`)를 입력으로 쓰고, 타깃은 unit health를 4 die에 broadcast한 값.
- die 예측 → **unit 집계** 후 unit RMSE로 평가/선택. 집계는 mean(기본) 또는
  position-weighted(w1~w4, Optuna 탐색).
- 메타 CV는 `ufs_serial` 단위로 분할 — 같은 unit의 4 die가 train/holdout에 섞이면 누수.
- die 행은 (ufs_serial, position) 정렬이라 4행=1 unit → reshape(-1, 4)로 집계.
- v3-v1은 SHAP extra-cache 미지원(캐시가 unit-level). base 좁히기 `--variant_filter`/
  `--dedup_runs` 추가.
...
RESULT_DIR = PROJECT_ROOT / "4_output" / "04_stacking" / "results_extreme_v3"
RESULT_DIR.mkdir(parents=True, exist_ok=True)

KEY_COL = "ufs_serial"
DEFAULT_SEED = 42
_VAL_GAP_PENALTY: float = 0.0   # main()에서 args.val_gap_penalty로 설정
_USE_ISO: bool = True           # main()에서 not args.no_iso로 설정 (--no_iso면 False)
KNOWN_STRONG_SUBSET = [
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--variant_filter", type=str, default=None,
                   help="특정 variant만 사용 (예: hp 또는 hp,pphp). 미지정=전체.")
    p.add_argument("--dedup_runs", action="store_true",
                   help="모델 식별자별로 oof 최저 런 1개만 유지 (001/002 중복 제거).")
    p.add_argument("--pos_weights", action=argparse.BooleanOptionalAction, default=True,
                   help="Optuna가 die→unit position 집계가중 w1~w4를 탐색 (default ON).")
    p.add_argument("--no_iso", action="store_true",
                   help="IsotonicRegression 보정 생략 → 메타러너 raw 연속 출력 사용. "
                        "iso의 값 구간화(bucketing) 회피. zero 구간은 zero_tau가 처리.")
    return p.parse_args()
...
def final_sort_key(rec: Record, select_by: str):
    """save_outputs 최종 랭킹·best 선정용 정렬 키.

    select_by=meta_cv_oof: meta_cv_oof_rmse가 있는 record는 (0, mcv, oof)로 앞에,
    없는(=oof로 fallback될) record는 (1, oof, 0)로 뒤에 — in-sample oof로 채점된
    record가 정직한 CV record를 제치고 best로 뽑히는 것을 막는다.
    그 외(select_by=oof/val): score_rec 순.
    """
    if select_by == "meta_cv_oof":
        if not math.isnan(rec.meta_cv_oof_rmse):
            return (0, rec.meta_cv_oof_rmse, rec.oof_rmse)
        return (1, rec.oof_rmse, 0.0)
    return (0, score_rec(rec, select_by), 0.0)
...
def discover_models(args: argparse.Namespace) -> list[dict]:
    ...
    models = []
    for dp in sorted(OUTPUT_DIR.rglob("oof_die.csv")):
        dirpath = dp.parent
    ...
    # --- variant / run 좁히기 (v3: --variant_filter, --dedup_runs) ---
    vf = getattr(args, "variant_filter", None)
    if vf:
        keep = {v.strip() for v in str(vf).split(",") if v.strip()}
        before = len(models)
        models = [m for m in models if m["variant"] in keep]
        print(f"[variant_filter={vf}] base 모델: {before} → {len(models)}")
    if getattr(args, "dedup_runs", False):
        best: dict[str, dict] = {}
        for m in models:
            parts = m["name"].split("__")
            ident = "__".join(parts[:-2])   # variant·run 제거 = 모델 식별자
            if ident not in best or m["oof_rmse"] < best[ident]["oof_rmse"]:
                best[ident] = m
        before = len(models)
        models = sorted(best.values(), key=lambda m: m["oof_rmse"])
        print(f"[dedup_runs] 모델 식별자별 oof 최저 1개: {before} → {len(models)}")
    return models


def build_base_matrix(models: list[dict], split: str):
    """base 모델의 die-level 예측으로 X 행렬 구성 (v3: die-level stacking).

    각 모델의 {split}_die.csv를 run_wf_xy(die key)로 정합하고, 행을
    (ufs_serial, position) 순으로 정렬 → 4행이 한 unit, reshape(-1, 4)로 집계 가능.

    반환:
        X      (4*n_unit, n_model)  die-level 예측 행렬
        y_die  (4*n_unit,)          unit health를 4 die에 broadcast한 타깃
        keys   (n_unit,)            unit별 ufs_serial (정렬 순서)
    """
    # 1) position을 가진 모델에서 canonical die 순서 확보 ((ufs_serial, position) 정렬)
    canon = None
    for m in models:
        df = pd.read_csv(m["path"] / f"{split}_die.csv")
        if "position" in df.columns:
            canon = df[[KEY_COL, "run_wf_xy", "position", "health"]].copy()
            break
    if canon is None:
        df = pd.read_csv(models[0]["path"] / f"{split}_die.csv")
        canon = df[[KEY_COL, "run_wf_xy", "health"]].copy()
        canon["position"] = canon.groupby(KEY_COL)["run_wf_xy"].rank(method="first").astype(int)
        print(f"  [build_die_matrix/{split}] position 컬럼 없음 → run_wf_xy rank로 대체")
    canon = canon.sort_values([KEY_COL, "position"]).reset_index(drop=True)

    sizes = canon.groupby(KEY_COL).size()
    if not (sizes == 4).all():
        bad = sizes[sizes != 4]
        raise RuntimeError(f"{split}: die가 4개가 아닌 unit {len(bad)}개 (예: {bad.head(3).to_dict()})")

    # unit health는 4 die에 동일하게 broadcast돼 있어야 함 (unit_y가 [:,0]만 취함)
    hv = canon["health"].values.astype(float).reshape(-1, 4)
    if not np.allclose(hv, hv[:, [0]], rtol=0, atol=1e-12, equal_nan=True):
        raise RuntimeError(f"{split}: unit 내 health가 die마다 다름 — unit 타깃 broadcast 가정 위반")

    die_order = canon["run_wf_xy"].tolist()
    y_die = canon["health"].values.astype(float)
    keys = canon[KEY_COL].values[::4].tolist()    # unit별 ufs_serial

    # 2) 각 모델 die 예측을 canonical 순서로 정합 (pred 없으면 prob)
    cols = []
    for m in models:
        df = pd.read_csv(m["path"] / f"{split}_die.csv")
        pcol = "pred" if "pred" in df.columns else ("prob" if "prob" in df.columns else None)
        if pcol is None:
            raise RuntimeError(f"{m['name']}/{split}_die.csv: pred/prob 컬럼 없음 ({list(df.columns)})")
        arr = df.set_index("run_wf_xy")[pcol].reindex(die_order).values.astype(float)
        if np.isnan(arr).any():
            raise RuntimeError(f"{m['name']}/{split}: die key 정합 실패 (NaN {int(np.isnan(arr).sum())}개)")
        cols.append(arr)
    X = np.column_stack(cols)
    return X.astype(float), y_die, keys
...
def agg_die_to_unit(die_pred, agg_w=None):
    """die-level 예측 → unit-level 집계.

    die 행은 (ufs_serial, position) 4-연속 정렬이므로 reshape(-1, 4)이면 한 행이 한 unit.
    agg_w=None → 단순 평균(mean), 아니면 position 가중평균(w는 합이 1로 정규화).
    """
    u = np.asarray(die_pred, dtype=float).reshape(-1, 4)
    if agg_w is None:
        return u.mean(axis=1)
    w = np.asarray(agg_w, dtype=float)
    return u @ (w / w.sum())


def unit_y(y_die):
    """die-level broadcast 타깃 → unit-level (한 unit 내 4값 동일 → 첫 값)."""
    return np.asarray(y_die, dtype=float).reshape(-1, 4)[:, 0]
...
def apply_iso(raw_oof, raw_val, raw_test, y_oof, iso_weight=1.0, zero_tau=0.0):
    ...
    if _USE_ISO:
        ...
        # --no_iso: iso 보정 생략 → 메타러너 raw 연속 출력 그대로
        oof, val, test = raw_oof, raw_val, raw_test
...
def compute_meta_cv_oof(use_cols, arrays, n_splits=5, seed=42, alpha=1e-5, agg_w=None) -> float:
    """die-level OOF 행렬에서 unit-grouped CV: ridge+iso를 train die에 fit,
    holdout die 예측 → unit 집계 → unit RMSE.

    use_cols: subset에서 고른 base index들 (+ extra). X_oof[:, use_cols]는 die-level.
    CV fold는 unit 단위로 나눈다 — 같은 unit의 4 die가 train/holdout에 섞이면 누수.
    """
    X_oof, y_oof = arrays[0], arrays[1]          # die-level
    Xo = X_oof[:, list(use_cols)]
    n_units = len(y_oof) // 4
    y_u = y_oof.reshape(-1, 4)[:, 0]
    pos = np.arange(4)
    kf = KFold(n_splits=n_splits, shuffle=True, random_state=seed)
    unit_meta = np.zeros(n_units)
    for tr_u, va_u in kf.split(np.arange(n_units)):
        tr_d = (tr_u[:, None] * 4 + pos).ravel()   # unit index → die row index
        va_d = (va_u[:, None] * 4 + pos).ravel()
        X_tr, y_tr = Xo[tr_d], y_oof[tr_d]
        X_va = Xo[va_d]
        mu = X_tr.mean(axis=0)
        sd = X_tr.std(axis=0)
        sd[sd == 0] = 1.0
        ...
        if _USE_ISO:
            iso = IsotonicRegression(out_of_bounds="clip", y_min=0)
            iso.fit(raw_tr, y_tr)
            pred_va = iso.transform(raw_va)        # die-level holdout 예측
        else:
            pred_va = raw_va                       # --no_iso: raw 그대로
        unit_meta[va_u] = agg_die_to_unit(pred_va, agg_w)   # → unit 집계
    return float(rmse(clip_nonneg(unit_meta), y_u))
...
def eval_fast(
    ...
    # die-level 메타 예측 → iso 보정 → unit 집계 → unit RMSE
    po, pv, pt = apply_iso(ro, rv, rt, y_oof, iso_weight=iso_weight, zero_tau=zero_tau)
    po_u = agg_die_to_unit(po, agg_w)
    pv_u = agg_die_to_unit(pv, agg_w)
    pt_u = agg_die_to_unit(pt, agg_w)
    rec = Record(
        tag=f"{stage}__{method}__k{len(base_cols)}",
        stage=stage,
        method=f"{method}+{'Iso' if _USE_ISO else 'raw'}",
        n_base=len(base_cols),
        n_extra=len(extra_idx),
        val_rmse=rmse(pv_u, unit_y(y_val)),
        test_rmse=rmse(pt_u, unit_y(y_test)),
        oof_rmse=rmse(po_u, unit_y(y_oof)),
        ...
        params={"alpha": alpha, "iso_weight": iso_weight, "zero_tau": zero_tau,
                "agg_w": (list(map(float, agg_w)) if agg_w is not None else None)},
    )
    return rec, pv_u, pt_u
...
def _unit_cv_splits(n_die, n_splits=5, seed=DEFAULT_SEED):
    """die 행을 unit 단위로 묶은 (train_die_idx, val_die_idx) 리스트.

    die 행이 (ufs_serial, position) 4-연속 정렬이므로 unit u → die row [4u..4u+3].
    ElasticNetCV 등 내부 CV에 넘겨 같은 unit의 4 die가 fold에 섞이는 누수를 막는다.
    """
    n_units = n_die // 4
    pos = np.arange(4)
    kf = KFold(n_splits=n_splits, shuffle=True, random_state=seed)
    splits = []
    for tr_u, va_u in kf.split(np.arange(n_units)):
        splits.append(((tr_u[:, None] * 4 + pos).ravel(),
                       (va_u[:, None] * 4 + pos).ravel()))
    return splits
...
def fit_enet_cv_raw(Xo, yo, Xv, Xt, seed=DEFAULT_SEED, positive=False):
    pipe = Pipeline([
        ...
        ("en", ElasticNetCV(
            ...
            cv=_unit_cv_splits(len(yo), 5, seed),   # unit-grouped (die 누수 방지)
            n_jobs=-1,
            max_iter=20000,
            random_state=seed,
            positive=positive,
...
def eval_enet(base_cols, extra_idx, arrays, names, extra_tags, seed, positive=False,
              stage="refit", agg_w=None):
    ...
    rec = Record(
        tag=f"{stage}__ENet{'Positive' if positive else ''}+{'Iso' if _USE_ISO else 'raw'}__k{len(base_cols)}",
        stage=stage,
        method=f"ENet{'Positive' if positive else ''}+{'Iso' if _USE_ISO else 'raw'}",
        n_base=len(base_cols),
        n_extra=len(extra_idx),
        val_rmse=rmse(pv_u, unit_y(y_val)),
        test_rmse=rmse(pt_u, unit_y(y_test)),
...
def eval_combo(base_cols, extra_idx, arrays, names, extra_tags, seed, stage="refit", agg_w=None):
    X_oof, y_oof, X_val, y_val, X_test, y_test = arrays
    use_cols = _combine_cols(base_cols, extra_idx)
    ...
    rec = Record(
        tag=f"{stage}__Combo+{'Iso' if _USE_ISO else 'raw'}__k{len(base_cols)}",
        stage=stage,
        method=f"Combo+{'Iso' if _USE_ISO else 'raw'}",
        n_base=len(base_cols),
        n_extra=len(extra_idx),
        val_rmse=rmse(pv_u, unit_y(y_val)),
        test_rmse=rmse(pt_u, unit_y(y_test)),
...
def run_optuna_search(args, extra_idx, arrays, names, extra_tags, alpha_grid):
    ...
    def objective(trial):
        ...
        agg_w = None
        if getattr(args, "pos_weights", False):
            raw_w = [trial.suggest_float(f"w{p}", 0.05, 1.0) for p in range(1, 5)]
            sw = sum(raw_w)
            agg_w = [r / sw for r in raw_w]   # position 집계가중 (합=1 정규화)
...
def _enet_fit_components(Xo, yo, seed, positive):
    ...
    pipe = Pipeline([
        ...
        ("en", ElasticNetCV(
            l1_ratio=[0.1, 0.3, 0.5, 0.7, 0.9, 1.0],
            alphas=np.logspace(-6, 0, 30),
            cv=_unit_cv_splits(len(yo), 5, seed),
            n_jobs=-1,
            max_iter=20000,
            random_state=seed,
            positive=positive,
...
def extract_weights(rec, extra_idx, arrays, names, extra_tags, args):
    ...
    out["iso"] = _iso_curve_for_weights(raw_oof, y_oof) if _USE_ISO else None
    return out
...
def dump_weights_for_run(run_dir: Path, top_n: int = 1) -> Path:
    ...
    args.variant_filter = saved_args.get("variant_filter", None)
    args.dedup_runs = bool(saved_args.get("dedup_runs", False))
    args.pos_weights = bool(saved_args.get("pos_weights", True))
    args.no_iso = bool(saved_args.get("no_iso", False))
...
def save_outputs(records, pred_cache, keys_val, keys_test, args, models, extra_tags, extra_col_count,
    ...
    records = sorted(records, key=lambda r: final_sort_key(r, args.select_by))
    slim = []
    for r in records:
        d = asdict(r)
    ...
    best = records[0]
    if args.select_by == "meta_cv_oof" and math.isnan(best.meta_cv_oof_rmse):
        print("[WARN] meta_cv_oof 계산된 record가 없음 → best를 oof_rmse 최저로 fallback")
    ...
    # ── 선택된 best record(save_outputs가 best_val/test.csv로 저장한 그 record)의
    #    메타 가중치를 별도 파일로 박제 (재현/디버깅용). select_by 기준으로 뽑힌 record.
    if arrays is not None and names is not None and extra_idx is not None:
        try:
            best_w = extract_weights(best, extra_idx, arrays, names, extra_tags, args)
            best_w["rank_in_run_by_oof"] = 1
            best_w["selection_criterion"] = f"min({args.select_by})"
            with open(run_dir / "best_weights.json", "w", encoding="utf-8") as f:
                json.dump({
                    "source_run": str(run_dir),
                    "selection_criterion": f"min({args.select_by})",
                    "args_select_by": args.select_by,
                    "args_val_gap_penalty": getattr(args, "val_gap_penalty", 0.0),
                    "top_n": 1,
                    "weights": [best_w],
                }, f, ensure_ascii=False, indent=2)
            print(f"Saved best meta-weights ({args.select_by}): {run_dir / 'best_weights.json'}")
        except Exception as e:
...
def main():
    global _VAL_GAP_PENALTY, _USE_ISO
    args = parse_args()
    _VAL_GAP_PENALTY = args.val_gap_penalty
    _USE_ISO = not args.no_iso
    if args.extra_cache:
        raise SystemExit("v3-v1은 --extra-cache 미지원 (SHAP 캐시는 unit-level). 빼고 실행하세요.")
    deadline = parse_deadline(args.deadline)
    ...
    print("=" * 92)
    print("squeeze_extreme_v3.py  (die-level stacking: base die preds → meta → unit 집계)")
    ...
    print(f"iso 보정: {'ON' if _USE_ISO else 'OFF (--no_iso: raw 연속 출력)'}")
    if deadline is not None:
    ...
    arrays = (X_oof, y_oof, X_val, y_val, X_test, y_test)
    print(f"\nShapes(die): oof={X_oof.shape} val={X_val.shape} test={X_test.shape} "
          f"(base={n_base}+extra={n_extra})")
    print(f"Units: oof={len(y_oof)//4} val={len(y_val)//4} test={len(y_test)//4}  "
          f"pos_weights={getattr(args, 'pos_weights', False)}")
    ...
    # 후보 subset을 oof proxy로 넓게 모은 뒤, select_by=meta_cv_oof면 meta_cv_oof로 재선정.
    # (search는 oof로 굴러가므로 refit 후보 풀까지 oof-bias가 남지 않도록 여기서 정직화)
    cand_cols = []
    seen = set()
    K_cand = max(args.top_refit * 6, 150)
    for rec in sorted(records, key=lambda r: r.oof_rmse):
        ...
        if len(cand_cols) >= K_cand:
            break
    if args.select_by == "meta_cv_oof":
        mcv_pre = {c: compute_meta_cv_oof(_combine_cols(c, extra_idx), arrays, seed=args.seed)
                   for c in cand_cols}
        unique_cols = sorted(cand_cols, key=lambda c: mcv_pre[c])[:args.top_refit]
        print(f"  [refit 후보] {len(cand_cols)} subset meta_cv_oof 계산 → 최저 {len(unique_cols)}개 refit")
    else:
        unique_cols = cand_cols[:args.top_refit]
    ...
    for rec in sorted(records, key=lambda r: final_sort_key(r, args.select_by)):
        if id(rec) in pred_cache:
            break
        pred = prediction_for_record(rec, extra_idx, arrays, names, extra_tags, args)
        if pred is None:
