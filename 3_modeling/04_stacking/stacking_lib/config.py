"""StackingConfig — 모든 파라미터를 한 dataclass에 집약.

노트북에서:
    from stacking_lib.config import StackingConfig
    cfg = StackingConfig(random_trials=2000, top_refit=30,
                          shap_caches=["shap_cache/02_reg_single__lgbm"], ...)
형태로 사용. die-level stacking의 모든 옵션 + SHAP X-stacking 옵션(shap_caches / shap_top_k /
shap_mode / shap_prefix_with_tag)을 한 곳에 모은다.
"""
...
def _default_project_root() -> Path:
    """이 파일 위치(stacking_lib/config.py)에서 프로젝트 루트를 자동 추론.

    경로: 3_modeling/04_stacking/stacking_lib/config.py → parents[3] = 프로젝트 루트.
...
class StackingConfig:
    ...
    **False면 raw 메타 출력을 그대로 사용**(non-negative clip만 적용) — 연속형 예측이 필요할 때.
    ...
    예: ('shap_cache/02_reg_single__lgbm', 'shap_cache/01_zit__zit_only_pearson__mu').
    ...
    """True면 SHAP 컬럼 이름 앞에 캐시 태그 prefix (예: '02_reg_single__lgbm::X146') —
    ...
    """프로젝트 루트. 기본값: 이 파일(stacking_lib/config.py) 위치에서 부모 디렉토리 3단계 위.
    (3_modeling/04_stacking/stacking_lib/config.py → .../04_stacking → .../3_modeling → 프로젝트 루트)
    노트북에서 cwd가 다르더라도 자동으로 올바른 경로를 잡는다. 명시적으로 덮어쓰려면 인자로 전달."""

    output_subdir: str = "04_stacking"
    """결과 저장 위치 (project_root / '4_output' / output_subdir / 'run_{ts}'). 버전 표기 없음(§5.3)."""
...
def parse_deadline(text: Optional[str]) -> Optional[datetime]:
    """StackingConfig.deadline 문자열 → datetime. None이면 None."""
    if not text:
        return None
    text = text.strip()
