...
def collect_base_predictions(
    ...
    rerun_mode = "kfold" if ens_cfg["force_kfold_mode"] else "single"
    n_folds_rerun = ens_cfg["n_folds_rerun"]
    ...
    for i, (clf_model, reg_model) in enumerate(candidates):
        ...
        rerun_result = rerun_best_trial(
            ...
            clf_early_stop=clf_early_stop * 2,
            reg_early_stop=reg_early_stop * 2,
            label_col=label_col,
            imbalance_method=imbalance_method,
            agg_funcs=agg_funcs,
            top_k_fixed=top_k_fixed,
