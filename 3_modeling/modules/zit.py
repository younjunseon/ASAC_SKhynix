...
#
# 출처: 3_modeling_이전자료/final/_temp_bagzit/zit_bag_hpo.ipynb (cell id="0e82b5ee")
# 원본은 노트북 inline subclass였음 → 모듈로 이관.
#
# Bag (unit) constraint 를 위한 ZITboost 변형:
#   - 매 EM iter 마다 same-unit dies 의 contribution sum 으로 unit_y 를 die-level 로
#     비례 분배 (B3 allocation)
#   - 학습 시 unit_id 인자 필수 (시그니처 차이 — hpo.refit_best 비호환)
#   - 노트북에서는 직접 fold loop + model.predict_components(X) 사용 권장
#
